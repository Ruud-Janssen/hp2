...
sortCol <- "SalePriceLog"   # sort column

# Save images in folder under graphics
fileDirPlot <- "graphs//eda//table lens"
fileNamePlot <- paste0("HousePricesKaggle_", sortCol) 

totalCol <- ncol(House.Prices.Kaggle.preprocessed)
sortColNr <- as.numeric(which( colnames(House.Prices.Kaggle.preprocessed) == sortCol))
totalPlots <- ceiling((totalCol - 1) / (totalNrOfColVisible - 1.0))

#Make a copy of the data and remove it all afterwards
data <- House.Prices.Kaggle.preprocessed
